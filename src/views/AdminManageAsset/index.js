        ...
        <Grid container spacing={gridSpacing}>
          <Grid item lg={3} sm={6} xs={12}>
            <ReportCard
              ...
              secondary="Total Contributions"
              color={theme.palette.warning.main}
              footerData={loggedUser.user_name === 'master' ? 'Total Pool Amount' : 'Invesments'}
          ...
          <Grid item lg={3} sm={6} xs={12}>
            <ReportCard
              primary={
                loggedUser.user_name === 'master'
                  ? parseFloat(loggedUser?.total_money_sent * ethPrice).toFixed(2) + ' $'
                  : parseFloat(loggedUser?.total_received_amount * ethPrice).toFixed(2) + ' $'
              }
              // primary={loggedUser?.total_received_amount}
              secondary={loggedUser.user_name === 'master' ? 'Utilized Amount' : 'Amount Recived'}
              color={theme.palette.primary.main}
              footerData={loggedUser.user_name === 'master' ? 'Invested on Compound Finance' : 'Total Amount Recived'}
          ...
          <Grid item lg={3} sm={6} xs={12}>
            <ReportCard
              primary={
                loggedUser.user_name === 'master'
                  ? parseFloat((loggedUser?.total_money - loggedUser?.total_money_sent) * ethPrice).toFixed(2) + ' $'
                  : +(parseFloat(loggedUser?.total_received_amount * ethPrice) * 1.04).toFixed(2) + ' $'
              }
              secondary="Unutilized Amount"
              color={theme.palette.success.main}
              footerData="Amount to be Invested"
              iconPrimary={MonetizationOnTwoTone}
          ...
          <Grid item lg={3} sm={6} xs={12}>
            <ReportCard
              primary={
                loggedUser.user_name === 'master'
                  ? (
                      parseFloat(loggedUser?.total_money_sent * ethPrice) * 1.04 -
                      parseFloat(loggedUser?.total_money_sent * ethPrice)
                    ).toFixed(4) + ' $'
                  : (parseFloat(loggedUser?.total_received_amount * ethPrice) * 1.04).toFixed(2)
              }
              secondary="Potential Interest"
              color={theme.palette.error.main}
              footerData={loggedUser.user_name ? 'Expected Earnings' : 'People in Your Community'}
              ...
              <CardContent>
                <Grid item lg={12} align="center">
                  ...
                  <Button
                    variant="contained"
                    size="small"
                    onClick={handleSendCompundFin}
                    disabled={amount <= 0 || amount === undefined ? true : false}
                  >
                    Invest
                  </Button>
                </Grid>
