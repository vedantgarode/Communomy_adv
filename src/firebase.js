...
export const getEthPriceDaily = async () => {
  ...
  const marketCapData = data1.data.market_caps;
  const latestMarketCap = marketCapData[marketCapData.length - 1][1];

  const VolCapData = data1.data.total_volumes;
  const VolMarketCap = VolCapData[marketCapData.length - 1][1];
  // Extracting daily prices
  const dailyPrices = data1.data.prices.map((price) => price[1]);
  //console.log("rj",dailyPrices)
  dailyPrices.push(latestMarketCap);
  dailyPrices.push(VolMarketCap);
  return dailyPrices;
};
...
export const add_familiy = async (user1, user2, u2Name) => {
  ...
  if (docSnap.exists()) {
    ...
    return 'Added to  Your Community!';
  }
};
//Admin T
...
export const invest_on_site = async (amount) => {
  ...
  const adminSnap = await getDoc(adminref);
  await updateDoc(adminref, {
    total_money_sent: parseFloat(adminSnap.data().total_money_sent) + parseFloat(amount)
  });
};
...
export const search_admin_transact = async () => {
  let transactions = [];
  try {
    const q = query(collection(db, 'admin_transact'), orderBy('Transaction_Time', 'desc'));
    const querySnapshot = await getDocs(q);
